...
private:
	...
	public:
			...
			mMenu(window, title.c_str()), mParent(parent)
		{
			...
			mMenu.addButton("VOLTAR", "accept", [this] { delete this; });

			if(mParent->mMultiSelect)
			{
				mMenu.addButton("SELECIONAR TODOS", "select all", [this, checkboxes] {
				...
				mMenu.addButton("SELECIONAR NENHUM", "select none", [this, checkboxes] {
					for(unsigned int i = 0; i < mParent->mEntries.size(); i++)
					{
...
private:
	...
	void onSelectedChanged()
	{
		if(mMultiSelect)
		{
			// display # selected
			std::stringstream ss;
			ss << getSelectedObjects().size() << " SELECIONADO(S)";
	...
	std::vector<HelpPrompt> getHelpPrompts() override
	{
		...
		if(!mMultiSelect)
			prompts.push_back(HelpPrompt("left/right", "mudar"));
